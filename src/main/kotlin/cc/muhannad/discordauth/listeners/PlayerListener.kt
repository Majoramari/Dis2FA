...
class PlayerListener(private val plugin: Dis2FAPlugin) : Listener {
    ...
    fun onPlayerPreLogin(event: AsyncPlayerPreLoginEvent) {
        ...
        if (plugin.configManager.getGuildId().isNotBlank()) {
            if (guild == null) {
                val message = plugin.configManager.formatKickComponent("discord-unavailable")
                event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER, message)
                return
            }
                ...
                if (banned) {
                    if (plugin.configManager.isApplyMinecraftBan()) {
                        Bukkit.getScheduler().runTask(plugin, Runnable {
                            ...
                            banList.addBan(profile, plugin.configManager.getBanReason(), null as java.util.Date?, "Dis2FA")
                        })
                    }
                    val message = plugin.configManager.formatKickComponent("discord-banned")
                    event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER, message)
                    return
                }
            }
            ...
            if ((plugin.configManager.requireMember() || allowedRoles.isNotEmpty()) && member == null) {
                val message = plugin.configManager.formatKickComponent(
                    "not-in-guild",
            ...
            if (member != null && !plugin.hasAllowedRole(member)) {
                val message = plugin.configManager.formatKickComponent("role-required")
                event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER, message)
                return
            }
        }
    ...
    fun onPlayerJoin(event: PlayerJoinEvent) {
        ...
        player.sendMessage(plugin.configManager.formatKickComponent("joined-linked"))
    }
