...
class DiscordListener(private val plugin: Dis2FAPlugin) : ListenerAdapter() {
    ...
    private val footerText = "Dis2FA made with ❤️ by MajorAmari"
    ...
    override fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        if (event.isFromGuild && plugin.configManager.isChatBridgeEnabled()) {
            ...
            if (channelId.isNotBlank() && event.channel.id == channelId) {
                ...
                val component = plugin.configManager.formatMinecraftChat(user, content)
    ...
    override fun onCommandAutoCompleteInteraction(event: CommandAutoCompleteInteractionEvent) {
        if (event.name != "randomizeid" || event.focusedOption.name != "player") return
    ...
    private fun buildStatusEmbed(): net.dv8tion.jda.api.entities.MessageEmbed {
        ...
        val tpsText = tps.joinToString(" / ") { formatTps(it) }
        val tpsBar = progressBar(tps.firstOrNull() ?: 0.0, 20.0)
